...
func NewEventDispatcher(workerCount int, queueSize int, executor *CommandExecutor) *EventDispatcher {
...
func (d *EventDispatcher) worker(id int) {
	...
	logInfo("Event worker %d started", id)

	for {
		select {
		case <-d.ctx.Done():
			logInfo("Event worker %d stopping", id)
			return
...
func (d *EventDispatcher) processEvent(job EventJob, workerID int) {
	...
	logInfo("Worker %d processing event %s", workerID, job.EventName)
	...
	go func() {
		...
		case <-ctx.Done():
			logInfo("Script execution for %s was canceled", job.EventName)
		}
	}()
	...
	case result := <-resultCh:
		if result.err != nil {
			logInfo("Script error [%s]: %v", job.EventName, result.err)
		} else if verbose && len(result.output) > 0 {
			logInfo("Script output [%s]: %s", job.EventName, limitString(string(result.output), 100))
		}
	case <-ctx.Done():
		logInfo("Script timeout [%s] after %v", job.EventName, job.Timeout)
	}
}
...
func (d *EventDispatcher) Dispatch(eventName, eventData, script string, timeout time.Duration) {
	...
	default:
		...
		logInfo("Event queue full, dropping event: %s", eventName)
	}
}
...
func (d *EventDispatcher) Shutdown(timeout time.Duration) {
	...
	select {
	case <-done:
		logInfo("Event dispatcher shut down gracefully")
	case <-time.After(timeout):
		logInfo("Event dispatcher shutdown timed out")
	}
}
