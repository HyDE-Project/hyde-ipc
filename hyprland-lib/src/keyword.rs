...
macro_rules! keyword {
    ($k:tt,$v:tt) => {
        CommandContent { flag: CommandFlag::Empty, data: format!("keyword {} {}", $k, $v) }
    };
    (g $l:tt) => {
        CommandContent { flag: CommandFlag::JSON, data: format!("getoption {}", $l) }
    };
}

impl Keyword {
    fn parse_opts(OptionRaw { option, int, float, str, set }: OptionRaw) -> crate::Result<Keyword> {
        ...
        if int_exists + float_exists + str_exists > 1 {
            hypr_err!(
                "Expected single value type, but received more than one! Please open an issue \
                 with hyprland-rs with the information: Option {{ option: {option}, int: {int:?}, \
                 float: {float:?}, str: {str:?}, set: {set} }}!"
            );
        }

        let value = match (int, float, str) {
            (Some(int), ..) => OptionValue::Int(int),
            (_, Some(float), _) => OptionValue::Float(float),
            (_, _, Some(str)) => OptionValue::String(str),
            (int, float, str) => hypr_err!(
                "Expected either an 'int', a 'float' or a 'str', but none of them is not \
                 received! Please open an issue with hyprland-rs with the information: Option {{ \
                 option: {option}, int: {int:?}, float: {float:?}, str: {str:?}, set: {set} }}!"
            ),
        };
